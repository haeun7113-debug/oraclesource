...
-- 서브쿼리
-- JOB_ID 가 ;'SA_MAN'인 사원들의 최대 연봉보다 높게 받는 사원들의 정보 조회
...
-- 사원번호,FIRST_NAME +LAST_NAME,부서번호,SALARY 조회
...
WHERE
	...
	WHERE
		D.DEPARTMENT_ID = 1700;
);

-- 스칼라서브쿼리 사용
-- 총 사원수 및 각 년도별로 고용된 사원들의 총 숫자 조회
-- 총사원수 2011입사자 2012입사자....
SELECT DISTINCT (SELECT COUNT(*) FROM EMPLOYEES E) AS "총 사원수",
